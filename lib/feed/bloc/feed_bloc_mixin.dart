...
typedef PaginatedFeedResult
    = Future<({int newPage, bool hasMore, List<ConBlock> blocks})>;

/// Represents an update to a page.
typedef ListPostMapper = List<ConBlock> Function(List<Post> post);
...
mixin FeedBlocMixin on Bloc<FeedEvent, FeedState> {
  ...
  /// Converts a list of [Post] objects to a list of [ConBlock] objects
  ...
  /// Inserts sponsored blocks into a list of [ConBlock].
  ///
  ...
  /// [ReceivePort].
  /// 5. Returns the inserted blocks as a list of [ConBlock].
  ///
  /// Example usage:
  /// ```dart
  /// final hasMore = true;
  ...
  Future<List<ConBlock>> insertSponsoredBlocks({
    required bool hasMore,
    required List<ConBlock> blocks,
  }) async {
    ...
    final insertedBlocks = await receivePort.first as List<ConBlock>;
  ...
  static Future<void> _computeSponsoredBlocks(List<dynamic> args) async {
    ...
    final blocks = args[2] as List<ConBlock>;
    ...
    final sponsored =
        sponsoredBlocksListJson.take(20).map(ConBlock.fromJson).toList();
